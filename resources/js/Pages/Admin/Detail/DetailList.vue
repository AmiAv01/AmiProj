...
const deleteProduct = (detail, index) => {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            try {
                router.delete('details/delete/' + detail.dt_id, {
                    onSuccess: (page) => {
                        this.delete(detail, index);
                        Swal.fire({
                            toast: true,
                            icon: "success",
                            position: "top-end",
            ...
            <div class="bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-hidden">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        <tbody>
                        <tr v-for="(detail, index) in details" :key="`${detail.dt_id}`"
                            class="border-b dark:border-gray-700">
                            <th scope="row"
                                class="px-4 py-3 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                {{ detail.dt_id }}</th>
                            <td class="px-4 py-3">{{ detail.fr_code }}</td>
                            <td class="px-4 py-3">{{ detail.dt_type }}</td>
                            <td class="px-4 py-3">{{ detail.lt_dt_acode }}</td>
                            <td class="px-4 py-3">{{ detail.dt_cargo }}</td>


                            <td class="px-4 py-3 flex items-center justify-end">

                                <button :id="`${detail.dt_id}-button`" :data-dropdown-toggle="`${detail.dt_id}`"
                                ...
                                <div :id="`${detail.dt_id}`"
                                    ...
                                    <div class="py-1">
                                        <a href="#" @click="deleteProduct(detail, index)"
