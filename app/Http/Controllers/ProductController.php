...
class ProductController extends Controller
{
    ...
    public function index(string $id): Response
    {
        ...
        if (!$detail->isEmpty()){
            return (!auth()->check()) ? Inertia::render('Card/GuestCard', ['detail' => $detail[0], 'isEmpty' => false])
                : Inertia::render('Card/Index', ['detail' => $detail[0], 'sameDetails' => $this->productService->getSameDetails($detail[0]->dt_typec, $id),
                'analogs' => $analogs, 'isEmpty' => false, 'imageUrl' => $this->productService->getImageUrl($detail[0]->dt_foto),
                'price' => $this->priceService->getPrice($detail[0]->dt_code, (auth()->user()->id))]);
        }
        $detailFromOems = $this->productService->getProductInfoFromOems($this->searchService, $id);
        return (!auth()->check()) ? Inertia::render('Card/GuestCard', ['detail' => $detailFromOems, 'isEmpty' => false]) : Inertia::render('Card/Index', ['detail' => $detailFromOems,
